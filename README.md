...
## 1. Keycloak Configuration (Prerequisites)
Before using the SDK, your Keycloak instance must be configured to support both user authentication and M2M communication.

### Step 1: Create a Client
For each microservice (e.g., `order-service`), you need a dedicated client in Keycloak:
1. Go to **Clients > Create Client**.
2. Set the **Client ID** to your service name.
3. Toggle **Client Authentication** to **ON** (This generates the Client Secret).
4. Toggle **Service Accounts Roles** to **ON** (This enables the Client Credentials grant for M2M calls).
5. Save.

### Step 2: Get the Client Secret
1. Navigate to the **Credentials** tab of your new client.
2. Copy the **Client Secret**. You will need this for your `.env` file.

### Step 3: Configure Client Scopes (M2M Permissions)
To restrict which services can interact with each other (e.g., allowing `order-service` to call `product-service` but nothing else):
1. Go to **Client Scopes > Create client scope**.
2. Name your scope (e.g., `products:read`) and save.
3. Go back to your calling client (`order-service`), and navigate to the **Client Scopes** tab.
4. Click **Add client scope**, select `products:read`, and add it as **Default** (or Optional).
*Note: The M2M token generated by the SDK's internal client will now automatically include this scope.*
